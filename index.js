...
app.get('/trending', isAuthenticated,(req, res) => {
...
app.get('/api/trending-books', (req, res) => {
    const limit = parseInt(req.query.limit) || 3; // Default to 3 trending books

    // SQL query to find the most borrowed books
    const trendingBooksQuery = `
        SELECT 
            b.id, b.title, b.author, b.genre, b.image, COUNT(br.book_id) AS borrow_count
        FROM 
            books b
        LEFT JOIN 
    ...
    db.query(trendingBooksQuery, [limit], (err, results) => {
        if (err) {
            console.error('Error fetching trending books:', err); // Log the error for debugging
            return res.status(500).json({ error: 'Internal Server Error' });
        }
        ...
        // Respond with the list of trending books
        res.json({
